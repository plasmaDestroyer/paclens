...
- **Git hooks do the checking.** `.githooks/` is tracked and wired up with `git config core.hooksPath .githooks` (local config, so a fresh clone must set it once). `pre-commit` runs `cargo fmt` (fixing and re-staging), then clippy and the tests, aborting the commit if either fails. `post-commit` runs `cargo install --path .` so `paclens` on PATH is the last commit. Both skip when no Rust or manifest file is staged. Escape hatches: `--no-verify`, `PACLENS_SKIP_HOOKS=1`, and `PACLENS_NO_INSTALL=1` for the install alone — worth using during a run of granular commits, since the fat-LTO relink costs ~80s each (#76).
