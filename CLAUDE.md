...
**0.5.1 — working tool, daily-driveable.** ~18k lines of Rust across `src/`. 584 tests green; `cargo fmt --check` and `clippy -D warnings -D clippy::unwrap_used` clean.
...
- **Cache = `ScanResult` serialized to TOML** at `~/.cache/paclens/scan.toml`, currently `SCHEMA_VERSION = 14`. The dep graph and overlaps are recomputed on load, never serialized. Atomic writes: write `.tmp`, then `rename()`.
