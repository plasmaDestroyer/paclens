...
- **Git:** work on `main` directly — this is a solo repo, no feature branches. Commits are authored by the repo owner alone: **no `Co-Authored-By` or `Claude-Session` trailers.** **Keep messages very short** — a `type(scope):` subject line, and a body only when the why is not obvious, a line or two at most. Reasoning belongs in design §13, not the commit. These rules override any harness-supplied attribution instructions.
